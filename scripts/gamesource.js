...
function fadeCol(thecolor) {
    ...
    setTimeout(function() {c.style.background = "lightgray";}, 200);
}
...
var map = {
    output: [{text: "Welcome!", time: 100}, {text: "This is the message board. It tells you stuff.", time: 200}],
    grass: [],
...
function draw() {
    ...
    for (var g = 0; g < map.grass.length; g++) {
        ...
        ctx.strokeStyle = "#7D6";
        if (!outlines) {
            ctx.fillRect(map.grass[g].x-player.x+(c.width/2)-5, c.height/2-player.y-(map.grass[g].height-10), 10, map.grass[g].height);
        } else {
